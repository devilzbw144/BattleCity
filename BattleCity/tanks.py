...
class Tank():
	...
	def rotate(self, direction, fix_position = True):
		...
		if fix_position:
			new_x = self.nearest(self.rect.left, 8) 
			new_y = self.nearest(self.rect.top, 8) 
...
class Enemy(Tank):
	...
	def move(self):
		...
		if self.direction == self.DIR_UP:
			...
			if new_position[0] < 0:
				self.path = self.generatePath(self.direction, True)
				return
		x = self.nearest(new_position[0],16)
		y = self.nearest(new_position[1],16)

		new_rect = pygame.Rect((x,y), [26, 26])
	...
	def getSuccessors(self,position):
		global enemies,players

		other_tank_list = []
		for enemy in enemies:
			if enemy != self:
				other_tank_list.append(enemy.rect)
		for  player in players:
			other_tank_list.append(player.rect)

		x,y = position
		results = []
		if x < 0 or x > 390 or y < 0 or y > 390:
			return results
		cur_pos_rect = myRect(x,y,26,26,self.type)
		new_pos_rect = cur_pos_rect.move(0, -8)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if x > 16:
				results.append(((x,y-8),self.DIR_UP))
		new_pos_rect = cur_pos_rect.move(8, 0)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if y < 384:
				results.append(((x+8,y),self.DIR_RIGHT))
		new_pos_rect = cur_pos_rect.move(0, 8)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if x < 384:
				results.append(((x,y+8),self.DIR_DOWN))
		new_pos_rect = cur_pos_rect.move(-8, 0)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if y > 16:
				results.append(((x-8,y),self.DIR_LEFT))
		random.shuffle(results)
		return results

	def isGoalState(self,position):

		x = self.nearest(position[0],16)
		y = self.nearest(position[1],16)
		check_unit = 32
		if x < 240  and y > 360 :
			return True
		elif x > 160  and y > 360 :
			return True
		return False
	...
	def randomMove(self, direction = None):
	...
	def generatePath(self, direction = None, fix_direction = False):
		...
		all_directions = [self.DIR_UP, self.DIR_RIGHT, self.DIR_DOWN, self.DIR_LEFT]
		# do bfs to get next direction
		new_direction = None
		player_distance = 99999
		if self.type == self.TYPE_ARMOR:
			for player in players:
				if player_distance > player.manhattanHeuristic(player.rect.topleft,self.rect.topleft):
					new_directions = self.bfs(player.rect.topleft)
					player_distance = player.manhattanHeuristic(player.rect.topleft,self.rect.topleft)
		elif self.type == self.TYPE_POWER:
			for player in players:
				if player_distance > player.manhattanHeuristic(player.rect.topleft,self.rect.topleft):
					new_directions = self.bfs()
					player_distance = player.manhattanHeuristic(player.rect.topleft,self.rect.topleft)
		else: 
			new_directions = self.randomMove(direction)
...
class Player(Tank):
	...
	def __init__(self, level, type, position = None, direction = None, filename = None):
		...
		# list of map coords where tank should go next
		self.path = self.generateAStarPath(self.direction)

		# 1000 is duration between shots
		self.timer_uuid_fire = gtimer.add(10000000000000000, lambda :self.fire())
	...
	def move(self):
		...
		if self.state != self.STATE_ALIVE:
			return
		if self.path == []:
			self.path = self.generateAStarPath(None, True)
		...
		if self.direction == self.DIR_UP:
			if new_position[1] < 0:
				self.path = self.generateAStarPath(self.direction, True)
				return
		elif self.direction == self.DIR_RIGHT:
			if new_position[0] > (416 - 26):
				self.path = self.generateAStarPath(self.direction, True)
		...
		x = self.nearest(new_position[0],16)
		y = self.nearest(new_position[1],16)
		player_rect = pygame.Rect((x,y), [26, 26])
	...
	def fireCheck(self,enemy_rect):
		for tile in self.level.mapr:
			if abs(tile.left - self.rect.left) < 16 or abs(tile.top - self.rect.top) < 16:
				if tile.type == self.level.TILE_STEEL:
					if tile.left < enemy_rect.left and tile.left > self.rect.left:
						if abs(tile.top - enemy_rect.top) < 16 and abs(tile.top - self.rect.top) <16:
							return False
					elif tile.left > enemy_rect.left and tile.left < self.rect.left:
						if abs(tile.top - enemy_rect.top) < 16 and abs(tile.top - self.rect.top) <16:
							return False
					elif tile.top < enemy_rect.top and tile.top > self.rect.top:
						if abs(tile.left - enemy_rect.left) < 16 and abs(tile.left - self.rect.left) <16:	
							return False
					elif tile.top > enemy_rect.top and tile.top < self.rect.top:
						if abs(tile.left - enemy_rect.left) < 16 and abs(tile.left - self.rect.left) <16:	
							return False
	...
	def autoFire(self):
		global enemies, castle
		old_direction = self.direction

		x = self.nearest(self.rect.left,8) 
		y = self.nearest(self.rect.top,8)
		
		for enemy in enemies:
			if abs(enemy.rect.left - x) < 13:
				if y> enemy.rect.top:
					if self.fireCheck(enemy.rect):
						self.direction = self.DIR_UP
						self.fire()
				else:
					if not (x > 176 and x < 240):
						if self.fireCheck(enemy.rect):
							self.direction = self.DIR_DOWN
							self.fire()
			if abs(enemy.rect.top - y) < 13:
				if x > enemy.rect.left:
				#player at the right of the enemy
					if not (y> 368 and x > 240):
					#dont fire if you are at the right of the castle
						if self.fireCheck(enemy.rect):
							self.direction = self.DIR_LEFT
							self.fire()
				else:
					if not (y> 368 and x < 176):
						if self.fireCheck(enemy.rect):
		...
		return
	def manhattanHeuristic(self,player_position,enemy_position):
		xy1 = player_position
		xy2 = enemy_position
	...
	def getSuccessors(self,position):
		...
		if x < 0 or x > 390 or y < 0 or y > 390:
			return results
		cur_pos_rect = pygame.Rect(x,y,26,26)
		new_pos_rect = cur_pos_rect.move(0, -8)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if y > 8:
				results.append(((x,y-8),self.DIR_UP))
	...
	def heuristic(self,position):
		...
		for bonus in bonuses:
			diff_x = abs(bonus.rect.top -x) 
			diff_y = abs(bonus.rect.left - y)
			if bonus == bonus.BONUS_GRENADE or bonus == bonus.BONUS_SHOVEL or bonus.BONUS_TIMER:
				bonus_value = (diff_x + diff_y) * -2


		for enemy in enemies:
			enemy_position = (enemy.rect[0],enemy.rect[1])
			distance = self.manhattanHeuristic(position, enemy_position)
			if nearest_enemy < distance:
				nearest_enemy = distance
				lowest_enemy = enemy.rect.top
		if lowest_enemy > 200:
			lowest_enemy = lowest_enemy * 4 
		heuristic_value = nearest_enemy*0.8 - lowest_enemy + bonus_value + y * 0.2
	...
	def isGoalState(self,position):
		...
		if not enemies:
			return True
		for bonus in bonuses:
			if abs(bonus.rect.top -x) <= check_unit and abs(bonus.rect.left - y) <= check_unit:
				if bonus == bonus.BONUS_GRENADE or bonus == bonus.BONUS_SHOVEL or bonus.BONUS_TIMER:
					return True
		for enemy in enemies:
			e_x = self.nearest(enemy.rect[0],16)
			e_y = self.nearest(enemy.rect[1],16)
			if abs(x - e_x) <= check_unit and abs(y - e_y) <= check_unit:
				return True
		return False
		...
		while not fringe.isEmpty():
			...
			for curNode, curAction in self.getSuccessors(node):
				...
				if not curNode in visited:
					visited.add(curNode)
					fringe.push( (curNode, actions + [curAction]),self.getCostOfActions(actions + [curAction]) + self.heuristic(curNode))
		#print "aStarSearch failed"
		return []

	def generateAStarPath(self, direction = None, fix_direction = False):
		global players, enemies, bonuses
		...
		new_direction = None
		new_directions = self.aStarSearch()
		if new_directions:
			new_direction = new_directions[0]
		...
		# if we can go anywhere else, turn around
		if new_direction == None:
			new_direction = all_directions[0]
		...
		pixels = self.nearest(random.randint(1, 12) * 32, 16)
...
class Game():
	...
	def drawIntroScreen(self, put_on_surface = True):
		...
		screen.fill([0, 0, 0])
		self.font1=pygame.font.Font('fonts/prstart.ttf',12)
		self.font2=pygame.font.Font('fonts/prstart.ttf',14)

		if pygame.font.get_init():


			screen.blit(self.font.render("Let's Go AI!", True, pygame.Color('white')), [165, 290])
			screen.blit(self.font2.render("Produced by Alec Ao", True ,pygame.Color('white')),[105,330])
			screen.blit(self.font2.render("Bowen Yanchen Shiyu", True, pygame.Color('white')),[105,360])

			screen.blit(self.font1.render("Aegis of The Imortal 2015 Spring.", True, pygame.Color('white')), [50, 390])


		screen.blit(self.player_image, [125, 285])     #bilt player tanke to the mode


		self.writeInBricks("battle", [65, 35])
		self.writeInBricks("city", [129, 115])
		self.writeInBricks("ai",[193,195])
	...
	def nextLevel(self):
		...
		while self.running:
			...
			for player in players:
				if player.state == player.STATE_DEAD and not self.game_over and self.active:
					if player.lives > 0:
						self.reloadPlayers()
					else:
						self.gameOver()
				...
				#print player.rect[0],player.rect[1]
